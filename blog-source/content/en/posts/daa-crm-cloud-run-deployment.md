## C:\Users\dbkr\workspace\daa-public-staging\blog-source\content\en\posts\daa-crm-cloud-run-deployment.md
...
description: "How we built and deployed a fully functional multi-tenant CRM for Detroit Automation Academy â€” from broken Go string literals to a live React registration portal on Google Cloud Run."
---
...
The first failure was a Go syntax error in `backend/main.go` â€” a raw newline embedded inside a string literal:
...
The React frontend had `App.tsx` and `EnrollmentForm.tsx` â€” but no `public/index.html`, no `src/index.tsx`, and an invalid `tsconfig.json`. `react-scripts build` requires all three.
...
The `deploy.sh` script ended with `terraform init && terraform apply` â€” but Terraform wasn't installed and the infra directory didn't exist. Rather than block, we replaced it with direct `gcloud run deploy` calls. Both backend and frontend were live in 60 seconds.

## Firestore Persistence

The original backend stored students in memory â€” wiped on every cold start. We rewrote `handlers/students.go` to use Firestore with a graceful in-memory fallback:
...
The admin dashboard is now protected by a PIN gate â€” staff enter the passcode once per session, stored in `sessionStorage`. The registration form remains fully public.
